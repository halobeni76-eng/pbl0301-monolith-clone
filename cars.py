...
def updatecar():
    rows = TBCars.select()
    return render_template('updatecar.html', appType=appType, rows=rows)

@app.route('/updatecarsave/<int:car_id>', methods=['GET', 'POST'])
def updatecarsave(car_id):
    car = TBCars.get_by_id(car_id)
    if request.method == 'POST':
        ...
        car.save()
        return redirect(url_for('readcar'))
    return render_template('updatecarform.html', appType=appType, car=car)
...
def searchcar():
    ...
    if keyword:
        rows = TBCars.select().where(
            ...
            TBCars.carmodel.contains(keyword)
        )
    return render_template('searchcar.html', appType=appType, rows=rows, query=keyword)
